...
def make_signal(start_time, end_time, values):
    if len(values) != end_time - start_time + 1:
        raise ValueError(
            f"Expected {end_time - start_time + 1} values for range "
            f"[{start_time}, {end_time}], got {len(values)}"
        )
...
def impulse_identity():
    return signal_from_samples(0,0, {0: 1})
...
def impulse_moving_average(length):
    samples = {i: 1 / length for i in range(length)}
