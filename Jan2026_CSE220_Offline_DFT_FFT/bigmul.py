...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    limbs = np.array([int(c) for c in chunks], dtype=np.int64)
    return sign, limbs
...
def multiply_transform(a, b, engine):
    ...
    aa = np.zeros(N, dtype=np.complex128)
    bb = np.zeros(N, dtype=np.complex128)

    aa[:len(a)] = a
    bb[:len(b)] = b

    A = engine.transform(aa)
    B = engine.transform(bb)

    spectrum_product = A * B

    convolution = engine.inverse(spectrum_product)
...
def multiply(text_a, text_b, method):
    ...
    if method == "dft":
        engine = DFTAnalyzer()
        product_limbs, N = multiply_transform(a, b, engine)
        ...
        try:
            ...
            raise NotImplementedError(
                "ArbitraryLengthFFT is not available in transforms.py"
            )
        ...
        product = engine.inverse(A * B)

        product_limbs = np.rint(product.real).astype(np.int64)
        product_limbs = product_limbs[:required]

        N = required

    elif method == "schoolbook":
        product_limbs = multiply_schoolbook(a, b)
        N = 0

    else:
        raise ValueError("Unknown multiplication method: %s" % method)

    product = from_limbs(result_sign, product_limbs)

    return product, N, a, b
...
def run_single(path, method, out_dir):
    ...
    report_lines = [
        ...
        "input: %s" % path,
        "method: %s" % method,
        "operand A digits: %d" % len(text_a.lstrip("+-")),
        "operand B digits: %d" % len(text_b.lstrip("+-")),
        "base: %d" % BASE,
        "base digits per limb: %d" % BASE_DIGITS,
    ...
    if not match:
        raise AssertionError(
            "MISMATCH: transform result does not equal Python verification"
        )
